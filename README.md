...
3. **Set Up Environment Variables**
   Create a `.env` file in the project root with the following content (replace with your actual Azure values):
   ```
   Understander_Endpoint=https://<your-endpoint>
   Understander_Key=<your-key>
   ```

4. **Run the Application**
   ```sh
   python AI_models/resume_understander.py
   ```

5. **Troubleshooting**
   - Ensure your Azure endpoint and key are correct.
   - If you encounter environment variable errors, double-check your `.env` file and that `python-dotenv` is installed.

6. **Notes**
   - The script expects a resume file at the path specified in the code. Update the path if needed.
   - For Azure setup, see the [official documentation](https://aka.ms/How-toguides).
